...
int TestarCadaLivroParaExcluir(Book *livros, int qntBookAtual, char *isbn){
    int i;
    for (i = 0; i < qntBookAtual; i++) {
        if (strcmp(livros[i].isbn, isbn) == 0) {
            return i;
...
int excluirTodosOsLivros(Book *livros, Loan *emprestimos, int *qntBookAtual, int qntLoanAtual) {
    int i, j;
    for (i = 0; i < *qntBookAtual; i++) {
        int excluirTudo = TestarCadaLivroParaExcluir(livros, *qntBookAtual, livros[i].isbn);
        if (excluirTudo >= 0) {
            if (LivroPossuiEmprestimo(emprestimos, qntLoanAtual, livros[i].isbn) == 0) {
                for (j = i; j < (*qntBookAtual - 1); j++) {
                    strcpy(livros[j].isbn, livros[j+1].isbn);
                    strcpy(livros[j].titulo, livros[j+1].titulo);
                    strcpy(livros[j].genero, livros[j+1].genero);
                    for (int k = 0; k < 100; k++)
                        strcpy(livros[j].autores[k], livros[j+1].autores[k]);
                    livros[j].qntautores[j] = livros[j+1].qntautores[j+1];
                    livros[j].numPaginas = livros[j+1].numPaginas;
                }
                (*qntBookAtual)--;
                i--;  // Ajuste o índice para reavaliar o livro movido para a posição atual
            }
        }
    }
...
int main(){
    ...
    int opMenu, opSubMenu, opLivros, opRelat, qntUser = 0, qntBook = 0, qntLoan = 0;
    do {
            ...
            case 2: {
                ...
                do {
                    ...
                    switch (opLivros) {
                        ...
                        case 6:
                            excluirTodosOsLivros(livros, emprestimos, &qntBook, qntLoan);
                            break;
                        case 7:
                            x = 0;
                            break;
